...
class Sphere : public Object
{
	public:
		...
		// set surface data such as normal and texture coordinates at a given point
		// on the surface
		...
		void getSurfaceData(const Vec3f & Phit, Vec3f & Nhit, Vec2f & tex) const
		{
			Nhit = Phit - center;
			Nhit.normalize();
			// In this particular case, the normal is simular to a point in a unit sphere
			// centered around the origin. We can thus use the normal coordinates to compute
			// the sperical coordinates of Phit.
...
// Returns true if the ray intersects an object. The variable tNear is set to the
// closest intersection distance and hitObject is a pointer to the intersected object.
// The variable tNear is set to infiity and hitObject is set null if no intersection
// was found
...
// Compute the color at the intersection point if any 
// return background color otherwise)
Vec3f castRay(
			...
			const std::vector<std::unique_ptr<Object>> & objects)
{
	...
	float	t; //this is the intersection distance from the ray origin to the hitpoint
	if (trace(orig, dir, objects, t, hitObject)) {
		Vec3f Phit = orig + dir * t;
		Vec3f Nhit;
		Vec2f tex;
		hitObject->getSurfaceData(Phit, Nhit, tex);
		// Use the normal and texture coordinates to shade the hitpoint.
			...
			const std::vector<std::unique_ptr<Object>> & objects)
{
	...
	// Don't forget to transform the ray origin (which is also the camera origin by
	// transforming the point with coordinates (0, 0, 0) to world-space using the
	// camera-to-world matrix.
	Vec3f orig;
	options.cameraToWorld.multVecMatrix(Vec3f(0), orig);
	for (uint32_t j = 0; j < options.height; ++j) {
		for (uint32_t i = 0; i < options.width; ++i) {
			// Generate primary ray direction. Compute the x and y position of the
			// ray in screen space. This gives a point on the image plane at z = 1.
			// From there, we simply compute the direction by normalized the 
			// resulting vec3f variable. This is similar to taking the vector between
			// the point on the image plane and the camera origin, which in camera
			// space is (0, 0, 0): ray.dir = normalize(Vec3f(x, y, -1) - Vec3f(0));
			float x = (2 * (i + 0.5) / (float)options.width - 1) * imageAspectRatio * scale;
            float y = (1 - 2 * (j + 0.5) / (float)options.height) * scale;
			// don't forget to transform the ray direction using the camera-to-world matrix
			Vec3f dir;
			options.cameraToWorld.multDirMatrix(Vec3<float>(x, y, -1), dir);
			dir.normalize();
